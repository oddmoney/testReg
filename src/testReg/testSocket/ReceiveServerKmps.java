...
public class ReceiveServerKmps {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				try {
					//threadPool.execute(new ConnectionWrapKmps(socket, sExcepIp));
					threadPool.execute(new ConnectionWrapKmps(serverSocket.accept()));
				} catch (Exception se) {
					se.printStackTrace();
				}
			}
...
class ConnectionWrapKmps implements Runnable {
	...
	public ConnectionWrapKmps(Socket socket) {
		this.socket = socket;
	}
	...
	public void run() {
		...
		try {
			 ...
			 * �쓳�떟 �뜲�씠�꽣 �깮�꽦
			 */
			resSb.append("0461004125700973   A006300000165464310                                                                                         1131051333309381O1010051322419445410500001017    131106000000262070KB 移대뱶         OK: 30002834                                    30002834    KB 移대뱶         020200030719031      �쟾�몴:留ㅼ엯�궗�젣異�                                                                                                                                               \n");
